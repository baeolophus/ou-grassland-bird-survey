...
#import bioclim layers
future_list <- list.files(path = "E:/Documents/college/OU-postdoc/research/grassland_bird_surveys/GIS_layers_original/he45bi50",
                          pattern = "tif$",
                          full.names = TRUE)
...
#show they are in different CRS with different extents
okcensus <- raster("E:/Documents/college/OU-postdoc/research/grassland_bird_surveys/ougrassland/gis_layers_processed/resample_census_utm_30m.tif")
extent(okcensus)
...
#write the new file to smaller files that I can import later  without re-processing
writeRaster(utm.future,
            filename = names(utm.future),
            format = "GTiff",
            bylayer = TRUE,
            overwrite = TRUE)
...
#Create masking raster y out of Oklahoma.
blank_oklahoma_census<-raster("E:/Documents/college/OU-postdoc/research/grassland_bird_surveys/ougrassland/gis_layers_processed/nlcd_processing/nlcd_cropped_to_ok_census/nlcd_ok_utm14_okmask.tif")
...
effort.length.ok.census <- rasterize(x = state,
                                     y = blank_oklahoma_census,
                                     field = numbers$meanlength,
                                     background = NA)

effort.time.ok.census <- rasterize(x = state,
                                     y = blank_oklahoma_census,
                                     field = numbers$meantime,
                                     background = NA)
...
time_of_day <- rasterize(x = state,
                                   y = blank_oklahoma_census,
                                   field = 7,
                                   background = NA)

writeRaster(time_of_day,
            filename = "time_of_day.tif",
            format="GTiff",
            overwrite = TRUE)
